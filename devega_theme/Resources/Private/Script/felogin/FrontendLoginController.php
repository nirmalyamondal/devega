            ...
            if (!$compareHash || !$compareHash[1] || $compareHash[0] < time() || $hash[0] != $compareHash[0] || md5($hash[1]) != $compareHash[1]) {
                ...
                if (isset($postData['changepasswordsubmit'])) {
                    if (strlen($postData['password1']) < $minLength) {
                        ...
                        $done = true;
                        $subpartArray['###CHANGEPASSWORD_FORM###'] = '';
                        /*$markerArray['###BACKLINK_LOGIN###'] = '<h2>Ihr Passwort wurde geändert</h2><p>Bitte melden Sie sich nun mit Ihrer E-Mail-Adresse und Ihrem neuen Passwort an. Sie können jetzt Ihre Konfiguration abschließen.</p><span class="btn btn-short btn-primary btn-fl white-arrow-left">'.$this->getPageLink('Jetzt anmelden',
                            [$this->prefixId . '[redirectReferrer]' => 'off']
                        ).'</span>';*/
						$markerArray['###BACKLINK_LOGIN###'] = '<h2>Ihr Passwort wurde geändert</h2><p>Bitte melden Sie sich nun mit Ihrer E-Mail-Adresse und Ihrem neuen Passwort an. Sie können jetzt Ihre Konfiguration abschließen.</p><p>&nbsp;</p><a class="btn btn-short btn-primary btn-fl white-arrow-left" href="'.$this->pi_getPageLink($this->frontendController->id).'">Jetzt anmelden</a></span><p>&nbsp;</p>';
							//if($this->pi_getLL('ll_forgot_reset_message_emailSent') == 'redirect') {
								$linkToCurrentUrl = $this->pi_getPageLink($GLOBALS['TSFE']->id);
								header("Location:".$linkToCurrentUrl);
								exit();
							//}
                    }
                }
